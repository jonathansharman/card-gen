...
struct text {
	...
	text(nl::json const& j) : markup{j.at("markup").get<std::string>()}, font{j.at("font")}, size{j.at("size")} {}
};
...
struct image {
	...
	image(nl::json const& j) : path{j.at("path").get<std::string>()} {
